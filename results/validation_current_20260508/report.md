...
| Five-lag spatial distillation + corr checkpoint selection | 0.0113 | -0.0171 | -0.0094 | Corr-selected validation checkpoint did not generalize |
...
| Five-lag hybrid + corr checkpoint selection | 0.0202 | -0.0087 | -0.0094 | Cleaner selection metric, but no gain over pure five-lag |
...
Adding online bandpower features to each lag-by-channel LaBraM token produced the strongest fold-1 temporal correlation so far: ROI r rose to 0.0290 while shifted-null fell to -0.0280. However, the 3-fold repeat failed to preserve this gain: mean real ROI r was -0.0047 and mean shifted-null ROI r was -0.0027. Switching checkpoint selection from validation MSE to validation temporal correlation made the one-fold hybrid control clean again (real 0.0202, null -0.0087), but did not beat the pure five-lag spatial distiller. Applying corr selection to pure five-lag was also worse (real 0.0113, null -0.0171). The useful lesson is not that this hybrid is solved, but that raw waveform tokens alone may be missing an easier time-frequency route while simple validation-correlation checkpointing does not reliably transfer to held-out subjects.
