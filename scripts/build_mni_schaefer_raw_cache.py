...
def safe_npz_copy_with_target(src: Path, dst: Path, y: np.ndarray, target_path: Path, target_kind: str) -> CacheRow:
...
def extract_roi_timeseries(mni_bold, atlas_path: Path) -> np.ndarray:
    atlas_img = nib.load(str(atlas_path))
...
def extract_schaefer_for_bold(args: argparse.Namespace, dataset: str, subject: str, bold_path: Path, t1_path: Path) -> TargetRow:
    ...
    if target_path.exists() and not args.rebuild_target:
        ...
        return TargetRow(
            ...
            int(np.sum(np.std(y, axis=0) > 1e-6)),
    ...
    y = extract_roi_timeseries(warped, atlas_path)
    np.savez_compressed(
        target_path,
    ...
    return TargetRow(
        ...
        float(np.nanmean(np.std(y, axis=0))),
        int(np.sum(np.std(y, axis=0) > 1e-6)),
        f"motion={args.motion_transform};bold2t1={args.bold_transform};t1tomni={args.t1_transform}",
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    for i, row in enumerate(selected, start=1):
        ...
        try:
            ...
            target = np.load(target_row.target_path, allow_pickle=True)["Y"].astype(np.float32)
            ...
            y = target[idx[good]]
            if good.all():
                cache_rows.append(safe_npz_copy_with_target(src, dst, y, Path(target_row.target_path), "mni_proxy_ants_schaefer100"))
            else:
                item = {k: z[k] for k in z.files}
