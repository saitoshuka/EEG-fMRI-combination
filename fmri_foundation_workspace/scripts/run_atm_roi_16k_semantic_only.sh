...
TAG=${TAG:-atm_semantic_group_train_seed${SEED}_budget16540_n16540_d256_none_seed${SEED}}
LOG_FILE="$LOG_DIR/${TAG}.log"
RUN_DIR="$OUT_DIR/$TAG"

if [[ ! -s "$RESIDUAL_TRAIN" || ! -s "$RESIDUAL_TEST" ]]; then
  echo "Missing residual ROI targets: $RESIDUAL_TRAIN / $RESIDUAL_TEST" >&2
  exit 2
fi
...
echo "training semantic-only $TAG -> $LOG_FILE"
