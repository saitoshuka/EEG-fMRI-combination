...
TAG=${TAG:-atm_spatial_parcel_raw_strongroi_train_seed33_budget16540_n16540_d256_none_lam010_col001_sp010}
...
if [[ "${WAIT_FOR_MAIN_PIPELINE:-1}" == "1" ]]; then
  while tmux has-session -t atm_roi_16k_pipeline 2>/dev/null; do
    log "waiting for atm_roi_16k_pipeline to finish before strong raw parcel run"
    sleep 300
  done
fi
