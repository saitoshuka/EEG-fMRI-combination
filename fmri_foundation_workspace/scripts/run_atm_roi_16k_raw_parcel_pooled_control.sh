...
TAG=${TAG:-atm_spatial_pooled_parcel_raw_strongroi_train_seed33_budget16540_n16540_d256_none_lam010_col001_sp010}
LOG_FILE="$LOG_DIR/${TAG}.log"
RUN_DIR="$OUT_DIR/$TAG"

if [[ ! -s "$RAW_TRAIN" || ! -s "$RAW_TEST" ]]; then
  echo "Missing raw ROI targets: $RAW_TRAIN / $RAW_TEST" >&2
  exit 2
fi
...
echo "training pooled no-query control $TAG -> $LOG_FILE"
